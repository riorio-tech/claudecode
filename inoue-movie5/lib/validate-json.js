...
const ShotSchema = z.object({
  ...
  // v2: motionHint（旧 motion は後方互換で残す）
  motionHint: z.enum([
    'fast_drop_bounce', 'zoom_out_fast', 'slow_push_in', 'continuous_orbit',
    'gentle_sway', 'parallax_drift', 'slide_wipe_left', 'dolly_in_tilt_up',
    'micro_drift', 'slow_roll',
  ]).optional(),
  motion: z.enum(['zoom-in', 'zoom-out', 'slide-left', 'slide-right', 'static', 'flash']).optional(),
  angleHint: z.enum([
    // v2 angleHints
    'shake_impact', 'pull_back_reveal', 'dutch_angle', 'extreme_close',
    'overhead_flatlay', 'hand_hold_pov', 'orbit', 'hero_low_angle',
    'lifestyle_scene', 'split_comparison',
    // 旧 angleHints（後方互換）
    'wide', 'close', 'front', 'angle', 'scene',
  ]),
}).refine(d => d.motionHint != null || d.motion != null, {
  message: 'motionHint または motion のどちらかは必須',
});
...
export const ImageVariantsSchema = z.object({
  jobId: z.string().uuid(),
  videoIndex: z.number().int().min(0).max(9),
  variants: z.array(VariantSchema).min(3).max(10),
});
...
export const VideoClipsSchema = z.object({
  jobId: z.string().uuid(),
  videoIndex: z.number().int().min(0).max(9),
  clips: z.array(ClipSchema).min(3).max(10),
});
